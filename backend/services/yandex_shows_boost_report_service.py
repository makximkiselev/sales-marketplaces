...
from backend.services.store_data_model import replace_sales_shows_boost_report_rows_for_period

logger = logging.getLogger("uvicorn.error")

DEFAULT_SHOWS_BOOST_DATE_FROM = "2025-07-01"
DEFAULT_SHOWS_BOOST_DATE_TO = "2026-03-11"
...
def _is_target_sheet(sheet_name: str, payload: Any) -> bool:
    ...
    if "business_shows_boost_consolidated_campaigns" in name:
        return True
    if "кампан" in name:
        return True
    if isinstance(payload, dict):
        ...
        if (
            "отчёт по кампаниям" in local
            or "отчет по кампаниям" in local
            or "business_shows_boost_consolidated_campaigns" in local
        ):
            return True
    return False
...
async def _generate_report(*, business_id: str, campaign_id: str, api_key: str, date_from: str, date_to: str) -> str:
    url = f"{YANDEX_BASE_URL}/reports/shows-boost/generate"
    ...
    async with httpx.AsyncClient(timeout=90) as client:
        logger.warning(
            "[sales_shows_boost] shows boost generate request business_id=%s campaign_id=%s date_from=%s date_to=%s",
            business_id,
            campaign_id,
            date_from,
            date_to,
        ...
        logger.warning(
            "[sales_shows_boost] shows boost generate response business_id=%s campaign_id=%s status=%s",
            business_id,
            campaign_id,
            resp.status_code,
        )
        if resp.status_code >= 400:
            logger.warning(
                "[sales_shows_boost] shows boost generate error business_id=%s campaign_id=%s body=%s",
                business_id,
                campaign_id,
                resp.text[:800],
            )
    ...
    if not report_id:
        raise RuntimeError("Маркет не вернул reportId для shows boost report")
    return report_id
...
async def _wait_report_download_url(*, report_id: str, api_key: str) -> tuple[str, str, str]:
    ...
    async with httpx.AsyncClient(timeout=60) as client:
        for attempt in range(_REPORT_INFO_MAX_ATTEMPTS):
            ...
            logger.warning(
                "[sales_shows_boost] shows boost report info report_id=%s attempt=%s status=%s sub_status=%s file=%s",
                report_id,
                attempt + 1,
                status or "-",
                sub_status or "-",
            ...
            if status in {"FAILED", "ERROR"}:
                raise RuntimeError(f"Маркет не смог подготовить shows boost report: {data}")
            await asyncio.sleep(_REPORT_INFO_POLL_INTERVAL_SEC)
    raise RuntimeError(f"Не удалось дождаться shows boost report: {last_error or report_id}")


async def refresh_yandex_shows_boost_for_store(
    *,
    store_uid: str,
    campaign_id: str,
    date_from: str,
    ...
    for chunk_from, chunk_to in _iter_month_ranges(date_from, date_to):
        ...
        loaded = replace_sales_shows_boost_report_rows_for_period(
            store_uid=store_uid,
        ...
        total_rows += loaded
        logger.warning(
            "[sales_shows_boost] shows boost loaded store_uid=%s campaign_id=%s report_id=%s date_from=%s date_to=%s rows=%s file_name=%s",
            store_uid,
            campaign_id,
            report_id,
            chunk_from,
...
async def refresh_sales_shows_boost_history(
    *,
    date_from: str = DEFAULT_SHOWS_BOOST_DATE_FROM,
    date_to: str = DEFAULT_SHOWS_BOOST_DATE_TO,
    ...
    if not stores:
        raise RuntimeError("Не найдены магазины Яндекс.Маркета для shows boost report")
    ...
    async def _run_store(store: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            logger.warning(
                "[sales_shows_boost] shows boost refresh failed store_uid=%s campaign_id=%s error=%s",
                store_uid,
                campaign_id,
                exc,
            )
