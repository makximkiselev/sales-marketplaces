...
def _tracking_status_allowed(status: str, *, mode: str = "created") -> bool:
    ...
    if str(mode or "").strip().lower() == "delivery":
        return status_kind in {"delivered", "return"}
    return status_kind in {"delivered", "return", "open"}
...
def _planned_costs_for_row(row: dict[str, Any], ctx: dict[str, Any]) -> dict[str, float | None]:
    ...
    ads_source = "market_boost_fact" if ads_from_strategy else "none"
...
async def get_sales_overview_tracking(
    *,
    ...
    for row in order_rows:
        ...
        if status_kind == "return":
            bucket["return_count"] += 1
            day_bucket["return_count"] += 1
    ...
    for bucket in grouped.values():
        ...
        for period_key in sorted(bucket["periods"].keys()):
            ...
            periods.append({
                ...
                "coinvest_pct": round((float(period["coinvest_amount"]) / period_revenue * 100.0), 2) if period_revenue > 0 else 0.0,
        ...
        rows_payload.append({
            ...
            "coinvest_amount": round(float(bucket["coinvest_amount"]), 2),
            "coinvest_pct": round((float(bucket["coinvest_amount"]) / revenue * 100.0), 2) if revenue > 0 else 0.0,
            "ads_amount": round(float(bucket["ads_amount"]), 2),
