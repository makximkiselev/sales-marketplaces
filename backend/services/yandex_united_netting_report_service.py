...
from backend.services.store_data_model import replace_sales_united_netting_report_rows_for_period

logger = logging.getLogger("uvicorn.error")

DEFAULT_NETTING_DATE_FROM = "2025-07-01"
DEFAULT_NETTING_DATE_TO = "2026-03-11"
...
def _is_target_sheet(sheet_name: str, payload: Any) -> bool:
    ...
    if any(token in name for token in ("payment", "payments", "платеж", "transaction_date")):
        return True
    if isinstance(payload, dict):
        local = json.dumps(payload, ensure_ascii=False).lower()
        if any(token in local for token in ("отчет о платеж", "отчёт о платеж", "payment report", "payments report")):
            return True
    return False


def _iter_month_ranges(date_from: str, date_to: str) -> list[tuple[str, str]]:
...
def _walk_payload_rows(payload: Any, *, sheet_name: str) -> list[dict[str, Any]]:
    ...
    if isinstance(payload, dict):
        ...
        if list_keys:
            for key in list_keys:
                ...
                nested_rows = _walk_payload_rows(payload.get(key), sheet_name=nested_sheet)
                if nested_rows:
        ...
        with zipfile.ZipFile(io.BytesIO(content)) as zf:
            for name in sorted(zf.namelist(), key=lambda item: item.lower()):
                ...
                try:
                    ...
                    continue
                if not _is_target_sheet(name, payload):
                    continue
                parsed_rows.extend(_walk_payload_rows(payload, sheet_name=name))
    except zipfile.BadZipFile:
        try:
            payload = json.loads(content.decode("utf-8"))
            if _is_target_sheet("report.json", payload):
                parsed_rows.extend(_walk_payload_rows(payload, sheet_name="report.json"))
        except Exception:
            pass
    return parsed_rows
...
async def _generate_report(*, business_id: str, campaign_id: str, api_key: str, date_from: str, date_to: str) -> str:
    url = f"{YANDEX_BASE_URL}/reports/united-netting/generate"
    ...
    async with httpx.AsyncClient(timeout=90) as client:
        logger.warning(
            "[sales_netting] united netting generate request business_id=%s campaign_id=%s date_from=%s date_to=%s",
            business_id,
            campaign_id,
            date_from,
            date_to,
        ...
        logger.warning(
            "[sales_netting] united netting generate response business_id=%s campaign_id=%s status=%s",
            business_id,
            campaign_id,
            resp.status_code,
        )
        if resp.status_code >= 400:
            logger.warning(
                "[sales_netting] united netting generate error business_id=%s campaign_id=%s body=%s",
                business_id,
                campaign_id,
                resp.text[:800],
            )
    ...
    if not report_id:
        raise RuntimeError("Маркет не вернул reportId для united netting report")
    return report_id
...
async def _wait_report_download_url(*, report_id: str, api_key: str) -> tuple[str, str, str]:
    ...
    async with httpx.AsyncClient(timeout=60) as client:
        for attempt in range(_REPORT_INFO_MAX_ATTEMPTS):
            ...
            logger.warning(
                "[sales_netting] united netting report info report_id=%s attempt=%s status=%s sub_status=%s file=%s",
                report_id,
                attempt + 1,
                status or "-",
                sub_status or "-",
            ...
            if status in {"FAILED", "ERROR"}:
                raise RuntimeError(f"Маркет не смог подготовить united netting report: {data}")
            await asyncio.sleep(_REPORT_INFO_POLL_INTERVAL_SEC)
    raise RuntimeError(f"Не удалось дождаться united netting report: {last_error or report_id}")


async def refresh_yandex_united_netting_history_for_store(
    *,
    store_uid: str,
    campaign_id: str,
    date_from: str,
    ...
    for chunk_from, chunk_to in _iter_month_ranges(date_from, date_to):
        ...
        loaded = replace_sales_united_netting_report_rows_for_period(
            store_uid=store_uid,
        ...
        total_rows += loaded
        logger.warning(
            "[sales_netting] united netting loaded store_uid=%s campaign_id=%s report_id=%s date_from=%s date_to=%s rows=%s file_name=%s",
            store_uid,
            campaign_id,
            report_id,
            chunk_from,
...
async def refresh_sales_united_netting_history(
    *,
    date_from: str = DEFAULT_NETTING_DATE_FROM,
    date_to: str = DEFAULT_NETTING_DATE_TO,
    store_uids: list[str] | None = None,
) -> dict[str, Any]:
    from_date = str(date_from or "").strip() or DEFAULT_NETTING_DATE_FROM
    to_date = str(date_to or "").strip() or DEFAULT_NETTING_DATE_TO
    stores = [
        store
    ...
    if not stores:
        raise RuntimeError("Не найдены магазины Яндекс.Маркета для united netting report")
    ...
    async def _run_store(store: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            logger.warning(
                "[sales_netting] united netting refresh failed store_uid=%s campaign_id=%s error=%s",
                store_uid,
                campaign_id,
                exc,
            )
    ...
    return {"ok": True, "date_from": from_date, "date_to": to_date, "stores": results, "errors": errors}
